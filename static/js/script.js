$(document).ready(function() {
    ...
    $('#tiktok-form').submit(function(e) {
        e.preventDefault();
        
        // Get URL from either mobile or desktop input
        var url = '';
        if ($(window).width() < 768) {
            url = $('#tiktok-url-mobile').val().trim();
        } else {
            url = $('#tiktok-url-desktop').val().trim();
        }
        ...
        if ($(window).width() < 768) {
            ...
            $('#loading-text-mobile').show();
            $('#download-button-mobile').prop('disabled', true);
        } else {
            $('#download-text-desktop').hide();
            $('#loading-text-desktop').show();
            $('#download-button-desktop').prop('disabled', true);
        }
        
        $('#error-alert').hide();
        
        // Check if we're on the MP3 page
        var isMP3Page = window.location.pathname.includes('/mp3');
        
        // Call the API to extract video info
        $.ajax({
            url: '/api/extract',
            type: 'POST',
            ...
            data: JSON.stringify({ 
                url: url,
                convert_to_mp3: isMP3Page // Add parameter for MP3 conversion
            }),
            success: function(response) {
                // Check if no video ID was found and try again with force_url_hash
                if (response.no_video_id || !response.video_id) {
                    $.ajax({
                        url: '/api/extract',
                        type: 'POST',
                        contentType: 'application/json',
                        ...
                        success: function(hashResponse) {
                            // Hide loading state
                            if ($(window).width() < 768) {
                                $('#download-text-mobile').show();
                                $('#loading-text-mobile').hide();
                                $('#download-button-mobile').prop('disabled', false);
                            } else {
                                $('#download-text-desktop').show();
                                $('#loading-text-desktop').hide();
                                $('#download-button-desktop').prop('disabled', false);
                            }
                            
                            // Process the response with hash
                            processVideoInfo(hashResponse, isMP3Page);
                        },
                        error: function(xhr) {
                            // Hide loading state
                            if ($(window).width() < 768) {
                            ...
                            // Show error message
                            let errorMessage = 'Failed to process TikTok URL even with URL hash';
                    ...
                    processVideoInfo(response, isMP3Page);
                }
            },
            error: function(xhr) {
                // Hide loading state
                ...
                // Show error message
                let errorMessage = 'Failed to process TikTok URL';
                if (xhr.responseJSON && xhr.responseJSON.error) {
                    errorMessage = xhr.responseJSON.error;
                }
                showError(errorMessage);
